...
namespace Dobby.Dialogs.WatchLists
{
    public class RemoveWatchListItemDialog : CancelAndHelpDialog
    {
        ...
        private async Task<DialogTurnResult> RemoveWatchListStep(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (deleted)
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Dobby successfully deleted item from the watchlist! Dobby makes you happy"), cancellationToken);
            }
            else
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Dobby failed to delete item from the watchlist! Dobby sorry"), cancellationToken);
            }
