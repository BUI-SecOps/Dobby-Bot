...
using Dobby.Dialogs.WatchLists;
using Dobby.Models;
...
namespace Dobby.Dialogs
{
    public class WatchlistDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ListAvailableWatchlistsStep(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (watchlists == null)
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Dobby didn't find any watchlists!"), cancellationToken);
        ...
        private async Task<DialogTurnResult> AddDeleteWatchlistStep(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            string message = $"Would you like Dobby to add or remove an item from the watchlist, or {DoneOption} to Cancel.";

            var promptOptions = new PromptOptions
            {
                Prompt = MessageFactory.Text(message),
                RetryPrompt = MessageFactory.Text("Please choose for Dobby to either add a new item to a watchlist or remove an existing one."),
